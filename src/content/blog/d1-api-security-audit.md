...
## 💥 【比較実験】あえて「脆弱な実装」にして攻撃してみた

セキュリティ対策の効果をより明確に検証するため、あえてコード上の防御ロジックを取り除いた**脆弱な実装バージョン**をローカル環境で一時的に作成し、同じテストスクリプトで攻撃を実行してみました。

### 1. XSSエスケープを解除した場合（Stored XSSの成立）
`escapeHtml` によるサニタイズ処理を取り除いて投稿を受け付けるようにコードを変更しました。
- **実験結果**: テストスクリプトから送信された `<script>alert('xss-author')</script>` や `<img src=x onerror=...>` が**そのままデータベースに保存され、取得用APIからも生のHTML/JSタグとして出力**されました。
- **リスク**: この状態でブラウザがレスポンスを表示すると、悪意あるJavaScriptが自動実行され、セッション情報の奪取やページの改ざんが発生します。

### 2. 管理者認可チェックを削除した場合（認可バイパスの成立）
DELETE API 内の `x-admin-secret` ヘッダーチェックをコメントアウトしました。
- **実験結果**: 認証ヘッダーを一切付与していないリクエストであっても **`200 OK` が返却され、指定したコメントが正常に削除**されてしまいました。
- **リスク**: 第三者が任意のコメントIDを指定するだけで、他人の投稿を自由に削除できてしまう深刻な脆弱性となります。

### 3. パラメータのクランプ処理を解除した場合（データ破壊の成立）
いいね追加API（POST `/api/likes`）の `Math.min(Math.max(...))` による数値制限を解除し、クライアントから送られてきた `increment` 数値をそのままDBに加算するように変更しました。
- **実験結果**: 
  - `increment: 999999` を送信すると、いいねカウントが一瞬で **1,000,000 以上に跳ね上がりました**。
  - `increment: -100` を送信すると、いいねカウントが **マイナスの数値に破壊**されました。
- **リスク**: データの信頼性が損なわれ、アプリケーションのビジネスロジックが破綻します。

### 実験結果の対比まとめ

| 診断項目 | 防御あり（安全な実装） | 防御なし（脆弱な実装） | 攻撃の成否 |
| :--- | :--- | :--- | :---: |
| **Stored XSS** | エスケープ済文字として出力 | 生のHTML/JSタグとして出力 | **攻撃成功 ⚠️** |
| **認可バイパス** | `403 Forbidden` で拒否 | `200 OK` (任意データ削除) | **攻撃成功 ⚠️** |
| **いいね数爆増** | 最大 50 に制御 | 1,000,000 超へ即座に加算 | **攻撃成功 ⚠️** |
| **いいね数マイナス化** | 最小 1 に制御 | マイナス数値へデータ破壊 | **攻撃成功 ⚠️** |

この比較実験により、今回実装している各防御層（サニタイズ、認可確認、サーバーサイドバリデーション）が、実際に攻撃を防ぐ上で不可欠であることが立証されました。
